...
class ContextManager {
  // Reduced default max concurrent workers to 3
  constructor({ maxConcurrentPerConnection = 3 } = {}) {
...
const FastActions = {
  ...
  async waitForAny(page, selectors, timeout = 15000) {
  ...
  async run(task, taskFn) {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        ...
        sendLog(`[Worker ${task.workerId}] Recoverable glitch (${err.message}). Retrying task (${attempt + 1}/${this.maxRetries})...`, 'warn');
...
async function runRexifyWorkflow(page, context, task) {
  ...
  // Set resilient default timeouts (20s)
  ...
  await FastActions.waitForAny(page, ['input[type="email"]', 'input[name="email"]'], 15000);
  ...
  // STEP 3: Account Verification (5 attempts max)
  ...
  while (!isVerified && verifyAttempt < MAX_VERIFY_ATTEMPTS) {
    ...
    if (verifyAttempt > 1) {
      // DOM recovery delay between attempts to avoid detached frames
      await delay(800);
    }

    await FastActions.waitForAny(page, ['input[placeholder*="account number" i]', 'input[name*="account" i]'], 15000);
    ...
    // Fast status check loop (polling every 300ms up to 7s)
    ...
    if (status === 'success') {
      ...
      sendLog(`[Worker ${workerId}] Verification status '${status}'. Retrying attempt...`, 'warn');
      await delay(600);
    }
  }
...
app.post('/api/start', upload.single('csvFile'), async (req, res) => {
  try {
    ...
    // Context Manager now configured to max 3 concurrent tasks per connection
    const contextManager = new ContextManager({ maxConcurrentPerConnection: 3 });
    ...
    (async () => {
      ...
      const BATCH_SIZE = 3; // Reduced batch size from 5 to 3
      ...
      for (let i = 0; i < accountRows.length; i += BATCH_SIZE) {
        ...
        const promises = currentBatch.map(async (row, batchIdx) => {
          ...
          // Stagger worker launches slightly to relieve network spikes
          await delay(batchIdx * 350);
